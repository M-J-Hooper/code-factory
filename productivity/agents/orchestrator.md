...
State is stored in `~/workspace/plans/do/<run-id>/`, outside the repo. The `/do` skill creates this directory and the initial state file BEFORE dispatching to this orchestrator. The `<state_path>` in your dispatch prompt points to the FEATURE.md file.

**CRITICAL:** State files live in `~/workspace/plans/do/<run-id>/` — never in the repo. The `<workdir_path>` is where code changes happen; the `<state_path>` is where state files live. These are separate locations.
...
**Update protocol:**
- All state writes go to `~/workspace/plans/do/<run-id>/` — the directory containing the FEATURE.md from your dispatch prompt
...
**Working directory is already set up.** `<workdir_path>` is for code changes. State files live in `~/workspace/plans/do/<run-id>/` (from `<state_path>`).
...
| **Merge to base** | `git checkout <base>`, `git merge <branch>`, clean up worktree if applicable. |
| **Keep branch** | Report branch name and worktree path (if applicable). |
| **Discard** | Require typed confirmation "discard". Then clean up worktree if applicable, `git branch -D <branch>`. |
